...
fn p__implicit_(state: &mut ProgramState) {
    if format!("{}", state.stopper).trim() == format!("{}", 8).trim() {
        println!("{}", format!("{}", "OK"));
    }
    if format!("{}", state.stopper).trim() != format!("{}", 9).trim() {
        { let _a: f64 = format!("{}", state.stopper).trim().parse().unwrap_or(0.0); let _b: f64 = format!("{}", 1).trim().parse().unwrap_or(0.0); state.stopper = format!("{}", _a + _b).cobol_into(); }
        // CALL 'callee2' USING 
    }
    return;
}
