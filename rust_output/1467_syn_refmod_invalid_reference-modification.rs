...
impl ProgramState {
    pub fn x_ab(&self) -> bool { format!("{}", self.x).trim() == "AB" }
}

/// Paragraph: _IMPLICIT_
fn p__implicit_(state: &mut ProgramState) {
    println!("{}", format!("{}", cobol_refmod(&format!("{}", state.x_ab), format!("{}", 1).trim().parse::<i64>().unwrap_or(1), format!("{}", 1).trim().parse::<i64>().unwrap_or(0))));
    return;
}
