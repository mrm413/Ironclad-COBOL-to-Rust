...
fn p_i(state: &mut ProgramState) {
    println!("{}", { let _rm_b = state.x.as_bytes().to_vec(); String::from_utf8(_rm_b[(state.i - 1) as usize..(state.i - 1 + 1) as usize].to_vec()).unwrap_or_default() });
    state.i = 4 as u32;
    println!("{}", { let _rm_b = state.x.as_bytes().to_vec(); String::from_utf8(_rm_b[(state.i - 1) as usize..(state.i - 1 + 1) as usize].to_vec()).unwrap_or_default() });
    state.i = 1 as u32;
    println!("{}", { let _rm_b = state.x.as_bytes().to_vec(); String::from_utf8(_rm_b[(1 - 1) as usize..(1 - 1 + state.i) as usize].to_vec()).unwrap_or_default() });
    state.i = 4 as u32;
    println!("{}", { let _rm_b = state.x.as_bytes().to_vec(); String::from_utf8(_rm_b[(1 - 1) as usize..(1 - 1 + state.i) as usize].to_vec()).unwrap_or_default() });
    { std::process::exit(0); }
}
