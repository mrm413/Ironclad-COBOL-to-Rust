...
fn p__implicit_(state: &mut ProgramState) {
    state.x = format!("{}", (format!("{}", format!("{}", 3)).trim().parse::<f64>().unwrap_or(0.0) + format!("{}", format!("{}", 4)).trim().parse::<f64>().unwrap_or(0.0))).cobol_into();
    if format!("{}", state.x).trim() != format!("{}", 7).trim() {
