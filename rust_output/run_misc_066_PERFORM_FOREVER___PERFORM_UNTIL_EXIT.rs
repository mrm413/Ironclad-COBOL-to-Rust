...
fn p_a01(state: &mut ProgramState) {
    state.indval = 0 as u32;
    // SKIP: unresolved condition
    if state.indval != 3 {
        println!("{}{}", "1: ", state.indval);
    }
    // PERFORM FOREVER — target not found, skipped
    state.indval += 1 as u32;
    if state.indval > 4 {
        // PERFORM END-IF — target not found, skipped
    }
    if state.indval != 5 {
        println!("{}{}", "2: ", state.indval);
    }
    { std::process::exit(0); }
}
