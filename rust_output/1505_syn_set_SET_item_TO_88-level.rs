...
impl ProgramState {
    pub fn x_wrong_init(&self) -> bool { format!("{}", self.x).trim() == "SPACE" }
    pub fn y_wrong_init(&self) -> bool { format!("{}", self.y).trim() == "LOW-VALUE" }
}

/// Paragraph: _IMPLICIT_
fn p__implicit_(state: &mut ProgramState) {
    state.x = "SPACE".to_string().cobol_into();
    state.y = "LOW-VALUE".to_string().cobol_into();
    std::process::exit(0);
}
