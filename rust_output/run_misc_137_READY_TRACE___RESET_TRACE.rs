...
fn p_section(state: &mut ProgramState) {
    if state._filler_5 == 0 {
        state._filler_5 = "".parse::<u32>().unwrap_or(0);
        callrec();
    }
    return;
}
...
/// External: CALL 'CALLREC'
fn callrec() {
    todo!("External subroutine 'CALLREC' not yet linked — implement or link the CALLREC module")
}
