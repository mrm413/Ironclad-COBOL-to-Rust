...
fn p__implicit_(state: &mut ProgramState) {
    state.xbyte = format!("{}", state.x"0d").cobol_into();
    if { let __v = format!("{}", state.x); __v.trim() != "" && __v.trim() != "0" } {
    }
    // ALPHABETIC-UPPER DISPLAY "Fail - Not alphabetic upper" END-DISPLAY END-IF
    state.xbyte = format!("{}", "A").cobol_into();
    if { let __v = format!("{}", state.x); __v.trim() != "" && __v.trim() != "0" } {
    }
    // NOT ALPHABETIC-UPPER DISPLAY "Fail - Alphabetic upper" END-DISPLAY END-IF
    std::process::exit(0);
}
