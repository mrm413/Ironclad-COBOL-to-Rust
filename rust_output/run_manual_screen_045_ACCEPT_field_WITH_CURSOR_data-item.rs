...
fn p_testme(state: &mut ProgramState) {
    println!();
    println!("{}{}{}", "(third column in field) below, then position it at", 2, 1);
    println!("{}{}{}", "the 'E' (fifth column) and press ENTER.", 3, 1);
    state.ws_x_20 = "ABCDEFG   ".into();
    state.cur_pos = 3 as u32;
    // ACCEPT WS-X-20 (console input)
    if (state.cur_pos == 5) && (state.cob_crt_status == 0) {
        return;
    }
    return;
}
