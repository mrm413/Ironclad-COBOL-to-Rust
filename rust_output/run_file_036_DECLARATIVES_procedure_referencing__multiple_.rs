...
/// MP0101
fn p_mp0101(state: &mut ProgramState) {
    test_file_open_input(state);
    // PERFORM P01 THRU P02 — range not found, skipped
    if state.z.trimmed() != "3" {
        println!("{}", state.z);
...
fn test_file_read(state: &mut ProgramState) -> Result<(), FileStatus> {
    match state._fh_test_file.read_line() {
        Ok(line) => {
            ...
            if _len > 0 { let _end = _len.min(0+10); state.test_rec = FixedString::from(String::from_utf8_lossy(&_bytes[0.._end])); }
...
fn test_file_write(state: &mut ProgramState) {
    ...
    { let _src = state.test_rec.as_bytes(); let _end = (0+10).min(_buf.len()); let _cl = _src.len().min(_end-0); _buf[0..0+_cl].copy_from_slice(&_src[.._cl]); }
