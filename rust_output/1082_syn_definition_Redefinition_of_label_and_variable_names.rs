...
fn p__implicit_(state: &mut ProgramState) {
    // MAIN SECTION
    state.word = format!("{}", 0).cobol_into();
    p_word(state);
    std::process::exit(0);
    // RETURNING WORD
    // WORD SECTION
    { let _a: f64 = format!("{}", state.word).trim().parse().unwrap_or(0.0); let _b: f64 = format!("{}", 1).trim().parse().unwrap_or(0.0); state.word = format!("{}", _a + _b).cobol_into(); }
}

/// Stub for unresolved paragraph
fn p_word(state: &mut ProgramState) {
