...
pub struct ProgramState {
    /// X
    pub x: Decimal /* 1,1 Signed */,
    /// Y
    pub y: PackedDecimal<2> /* Signed */,
...
fn p_x(state: &mut ProgramState) {
    state.x += Decimal::from(1i64);
    if (i64::from(state.x) as f64) != 0.9f64 {
        println!("{}", state.x);
        state.x = (0.1).into();
        state.x -= Decimal::from(1i64);
        if (i64::from(state.x) as f64) != -0.9f64 {
            println!("{}", state.x);
            state.y = (-0.1).into();
            state.y += PackedDecimal::new(1i64);
            if (state.y.value() as f64) != 0.9f64 {
                println!("{}", state.y);
                state.y = (0.1).into();
                state.y -= PackedDecimal::new(1i64);
                if (state.y.value() as f64) != -0.9f64 {
