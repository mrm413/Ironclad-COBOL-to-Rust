...
fn p__implicit_(state: &mut ProgramState) {
    // CALL 'PROG2' USING state.low_value, state.high_value, state.space, state.quote, state.zero
    // CALL 'PROG2' USING &mut state.low_value, &mut state.high_value, &mut state.space, &mut state.quote, &mut state.zero
    // CALL 'PROG3' USING &mut state.null
    std::process::exit(0);
}
