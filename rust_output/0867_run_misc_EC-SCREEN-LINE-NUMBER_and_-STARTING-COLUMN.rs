...
pub struct ProgramState {
    ...
    pub invalid_line: FixedString<30>,
}


/// Paragraph: _IMPLICIT_
fn p__implicit_(state: &mut ProgramState) {
    println!("{}", format!("{}", state.invalid_line));
    if format!("{}", cobol_fn_exception_status()).trim() == format!("{}", "EC-SCREEN-LINE-NUMBER").trim() {
        // CONTINUE
    } else {
        return;
    }
    // RETURNING 1 END-IF DISPLAY INVALID-COL END-DISPLAY IF FUNCTION EXCEPTION-STATUS = "EC-SCREEN-STARTING-COLUMN" CONTINUE ELSE GOBACK RETURNING 2 END-IF GOBACK RETURNING 0
}
