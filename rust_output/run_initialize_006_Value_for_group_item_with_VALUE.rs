// Generated by Ironclad — Deterministic COBOL-to-Rust Transpiler
// Do not edit manually. Regenerate from COBOL source.


use cobol_runtime::FixedString;
use cobol_runtime::Decimal;
use cobol_runtime::PackedDecimal;
use cobol_runtime::FileStatus;
use cobol_runtime::CobolFile;
#[allow(unused_imports)]
use cobol_runtime::string_ops::*;
#[allow(unused_imports)]
use cobol_runtime::chrono_shim::*;


#[derive(Debug, Clone, Default, PartialEq)]
pub struct G {
    /// G2
    pub g2: [G2; 3],
}
impl std::fmt::Display for G {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "")
    }
}
impl From<&str> for G {
    fn from(_s: &str) -> Self { Self::default() }
}
impl From<String> for G {
    fn from(_s: String) -> Self { Self::default() }
}
impl G {
    pub fn as_str(&self) -> String { format!("{}", self) }
    pub fn as_bytes(&self) -> Vec<u8> { format!("{}", self).into_bytes() }
    pub fn as_bytes_mut(&mut self) -> Vec<u8> { format!("{}", self).into_bytes() }
    pub fn trimmed(&self) -> String { format!("{}", self).trim_end().to_string() }
}

#[derive(Debug, Clone, Default, PartialEq)]
pub struct G2 {
    /// X
    pub x: u32,
    /// Y
    pub y: Y,
    /// Z
    pub z: FixedString<1>,
}
impl std::fmt::Display for G2 {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{}{}{}", self.x, self.y, self.z)
    }
}
impl From<&str> for G2 {
    fn from(_s: &str) -> Self { Self::default() }
}
impl From<String> for G2 {
    fn from(_s: String) -> Self { Self::default() }
}
impl G2 {
    pub fn as_str(&self) -> String { format!("{}", self) }
    pub fn as_bytes(&self) -> Vec<u8> { format!("{}", self).into_bytes() }
    pub fn as_bytes_mut(&mut self) -> Vec<u8> { format!("{}", self).into_bytes() }
    pub fn trimmed(&self) -> String { format!("{}", self).trim_end().to_string() }
}

#[derive(Debug, Clone, Default, PartialEq)]
pub struct Y {
    /// Y-REC
    pub y_rec: [YRec; 5],
}
impl std::fmt::Display for Y {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "")
    }
}
impl From<&str> for Y {
    fn from(_s: &str) -> Self { Self::default() }
}
impl From<String> for Y {
    fn from(_s: String) -> Self { Self::default() }
}
impl Y {
    pub fn as_str(&self) -> String { format!("{}", self) }
    pub fn as_bytes(&self) -> Vec<u8> { format!("{}", self).into_bytes() }
    pub fn as_bytes_mut(&mut self) -> Vec<u8> { format!("{}", self).into_bytes() }
    pub fn trimmed(&self) -> String { format!("{}", self).trim_end().to_string() }
}

#[derive(Debug, Clone, Default, PartialEq)]
pub struct YRec {
    /// Y1
    pub y1: u32,
    /// Y2
    pub y2: FixedString<1>,
}
impl std::fmt::Display for YRec {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{}{}", self.y1, self.y2)
    }
}
impl From<&str> for YRec {
    fn from(_s: &str) -> Self { Self::default() }
}
impl From<String> for YRec {
    fn from(_s: String) -> Self { Self::default() }
}
impl YRec {
    pub fn as_str(&self) -> String { format!("{}", self) }
    pub fn as_bytes(&self) -> Vec<u8> { format!("{}", self).into_bytes() }
    pub fn as_bytes_mut(&mut self) -> Vec<u8> { format!("{}", self).into_bytes() }
    pub fn trimmed(&self) -> String { format!("{}", self).trim_end().to_string() }
}

#[derive(Debug, Clone)]
pub struct ProgramState {
    /// G
    pub g: G,
    /// G2
    pub g2: [G2; 3],
    /// X
    pub x: u32,
    /// Y
    pub y: Y,
    /// Y-REC
    pub y_rec: [YRec; 5],
    /// Y1
    pub y1: u32,
    /// Y2
    pub y2: FixedString<1>,
    /// Z
    pub z: FixedString<1>,
    /// RETURN-CODE special register
    pub return_code: i32,
    /// TALLY special register
    pub tally: i32,
    /// SORT-RETURN special register
    pub sort_return: i32,
    /// COB-CRT-STATUS special register
    pub cob_crt_status: i32,
    /// DEBUG-ITEM special register
    pub debug_item: FixedString<80>,
    /// NUMBER-OF-CALL-PARAMETERS special register
    pub number_of_call_parameters: i32,
    /// WHEN-COMPILED special register
    pub when_compiled: FixedString<16>,
}

impl Default for ProgramState {
    fn default() -> Self {
        Self {
            g: Default::default(),
            g2: Default::default(),
            x: Default::default(),
            y: Default::default(),
            y_rec: Default::default(),
            y1: Default::default(),
            y2: Default::default(),
            z: Default::default(),
            return_code: 0,
            tally: 0,
            sort_return: 0,
            cob_crt_status: 0,
            debug_item: Default::default(),
            number_of_call_parameters: 0,
            when_compiled: Default::default(),
        }
    }
}

/// G
fn p_g(state: &mut ProgramState) {
    if { let _s = format!("{}", state.g); _s.trim().to_string() } != "00 0 0 0 0  00 0 0 0 0  00 0 0 0 0  ".to_string() {
        println!("{}{}{}", "INITIALIZE failed: ", state.g, ".");
        println!("{}{}{}", "        should be: ", "00 0 0 0 0  00 0 0 0 0  00 0 0 0 0  .", "");
    }
    state.g = Default::default();
    if { let _s = format!("{}", state.g); _s.trim().to_string() } != "09Y9Y9Y9Y9YZ09Y9Y9Y9Y9YZ09Y9Y9Y9Y9YZ".to_string() {
        println!("{}{}{}", "INIT VALUE failed: ", state.g, ".");
        println!("{}{}{}", "        should be: ", "09Y9Y9Y9Y9YZ09Y9Y9Y9Y9YZ09Y9Y9Y9Y9YZ.", "");
    }
    { std::process::exit(0); }
}

fn main() {
    let mut state = ProgramState::default();
    p_g(&mut state);
}

