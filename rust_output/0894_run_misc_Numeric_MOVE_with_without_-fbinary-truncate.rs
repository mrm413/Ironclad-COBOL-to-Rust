...
fn p__implicit_(state: &mut ProgramState) {
    state.x = format!("{}", 30000).cobol_into();
    p_check_x_val(state);
    state.x = format!("{}", 30000).cobol_into();
    p_check_x_val(state);
    state.x = format!("{}", 0).cobol_into();
    { let _a: f64 = format!("{}", state.x).trim().parse().unwrap_or(0.0); let _b: f64 = format!("{}", 30000).trim().parse().unwrap_or(0.0); state.x = format!("{}", _a + _b).cobol_into(); }
    p_check_x_val(state);
    return;
    // CHECK-X-VAL SECTION
    if (state.x == 30000) {
...
fn p_check_x_val(state: &mut ProgramState) {
