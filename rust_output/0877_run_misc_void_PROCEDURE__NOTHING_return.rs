...
fn p__implicit_(state: &mut ProgramState) {
    state.return_code = format!("{}", 42).cobol_into();
    // CALL 'callee' USING 
    // RETURNING NOTHING END-CALL
    if format!("{}", state.return_code).trim() != format!("{}", 42).trim() {
        println!("{}{}", format!("{}", "unexpected RETURN-CODE: "), format!("{}", state.return_code));
    }
    std::process::exit(0);
}
