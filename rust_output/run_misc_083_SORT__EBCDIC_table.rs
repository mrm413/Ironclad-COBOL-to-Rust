...
fn p_g(state: &mut ProgramState) {
    ...
    if { let _s = format!("{}", state.g); _s.trim().to_string() } != "abcde12345".to_string() {
        println!("{}", state.g);
        {
            let _bytes = state.z.as_bytes();
            let _len = _bytes.len();
            if _len > 0 { let _end = _len.min(0+1); state.g.tbl[0].x = FixedString::from(String::from_utf8_lossy(&_bytes[0.._end])); }
        }
        // // SORT TBL ON DESC X
        if { let _s = format!("{}", state.g); _s.trim().to_string() } != "54321edcba".to_string() {
            println!("{}", state.g);
            { std::process::exit(0); }
        }
    }
