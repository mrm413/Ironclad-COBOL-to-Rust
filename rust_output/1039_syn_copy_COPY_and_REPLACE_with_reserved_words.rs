...
fn p__implicit_(state: &mut ProgramState) {
    println!("{}{}{}", format!("{}", state.ws_in), format!("{}", state.in_a), format!("{}", state.in_ws));
    if format!("{}", state.ws_in).trim() == format!("{}", state.in_a).trim() {
        // COPY "proc.inc"
    }
    // REPLACING VAR BY IN-A IN INPUT-REC ==VAR-2== BY ==IN-WS ( 1:1 ) ==
    // END-IF
    std::process::exit(0);
}
