...
fn p_testme(state: &mut ProgramState) {
    println!();
    println!("{}{}{}", "column in the field below and it is empty, then", 2, 1);
    println!("{}{}{}", "enter 'AB', position after the 'B' and press ENTER.", 3, 1);
    state.ws_x_20 = "ABCDE     ".into();
    state.cur_pos = 8 as u32;
    // ACCEPT WS-X-20 (console input)
    if (state.cur_pos == 3) && (state.cob_crt_status == 0) {
        return;
    }
    return;
}
